...
export const isModeratorOrAdmin = async (req, res, next) => {
  try {
    ...
    // Si el usuario tiene alguno de los roles requeridos, pasamos al siguiente middleware
    if (hasRole) {
      next();
      return;
    }
    
    // Si el usuario no tiene ninguno de los roles requeridos, respondemos con un error 403

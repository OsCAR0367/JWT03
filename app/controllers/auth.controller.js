...
export const signup = async (req, res) => {
  try {
    ...
    // Asocia el rol encontrado al usuario (relacion muchos a muchos)
    await user.setRoles([userRole]);

    // Devuelve respuesta exitosa
    res.status(201).json({ message: "User registered successfully!" });
  } catch (error) {
    // Si ocurre un error, responde con codigo 500 y el mensaje del error
...
export const signin = async (req, res) => {
  try {
    ...
    // si la contrasena no es valida, responde con error 401
    if (!passwordIsValid) {
    ...
    // Genera un token JWT con el id del usuario y una expiración
    ...
    // Extrae los nombres de los roles para incluirlos en la respuesta
    const authorities = user.roles.map(role => `ROLE_${role.name.toUpperCase()}`);

    // Devuelve respuesta exitosa con datos del usuario y token
    res.status(200).json({
      id: user.id,
